...
wiring, a read grant Captain uses, and a self-verifying phone-home. Set this
up only if minutes-level reconciliation is not fresh enough for your use case.
...
yet live, so a deploy currently completes the setup steps in your account and
then fails loudly at the final verification step. Contact Captain for
...
r2/
  ...
  worker/
    src/index.ts            queue consumer + keyless read proxy + phone-home
...
Captain gives you `SYNC_ID` and a `CAPTAIN_SECRET`. You supply your account id and
bucket.
...
export CAPTAIN_SECRET=the-16-plus-char-secret-from-captain
# optional: export PREFIX=docs/   DEBUG=true   WORKER_NAME=...   QUEUE_NAME=...
./deploy.sh
```

`deploy.sh` preflights your tools and inputs, creates the queue + dead-letter
queue, deploys the Worker, sets the secret, wires the R2 event notification, then
phones home to Captain. It EXITS NON-ZERO unless Captain confirms enrollment, so a
successful run is a verified sync. Re-running is safe (idempotent).
...
Worker, and both queues; never touches the bucket itself).
...
The apply is self-verifying: `data.http.enroll` POSTs to Captain after all
resources exist, and a postcondition FAILS THE APPLY unless Captain returns
`verified: true`. On success, read the scoped credentials out to feed Captain's
R2 sync:
...
Terraform state for this stack holds, in plaintext: the Captain enrollment
secret (`var.captain_secret`) and the scoped R2 read token Terraform mints (its
id and the SHA-256-derived S3 secret). `sensitive = true` on those variables and
outputs only hides them from CLI output, it does NOT encrypt state.
...
`captain_secret` and re-apply to rotate the enrollment secret, which also
rewrites the Worker's `CAPTAIN_SECRET`. Rotation replaces the value going
...
| `deploy.sh` fails at phone-home (`verified=false`) | The Worker's enroll response is printed. It contains Captain's HTTP status and body. Usually Captain could not reach the read proxy, or Captain's enrollment endpoint is not yet live for your sync (see the note at the top). |
| Terraform apply fails on the enroll postcondition | The error prints Captain's HTTP status and first 400 bytes of the body. Re-apply after fixing; the deployment id is stable. |
